  ...
  return (
    <div className="flex flex-col h-full bg-sidebar">
      <div className="flex items-center gap-2 h-16 px-5 border-b border-sidebar-border bg-gradient-primary">
        ...
        <h1 className="text-base font-bold text-primary-foreground tracking-tight truncate">GM - GESTÃO GERAL</h1>
      </div>
      ...
      <div className="p-4 border-t border-sidebar-border space-y-3">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-sidebar-foreground truncate">{user?.email ?? 'GM - GESTÃO GERAL'}</p>
            <p className="text-xs text-muted-foreground truncate">{roleLabel}</p>
          </div>
          <Button variant="ghost" size="icon" onClick={signOut} aria-label="Sair" className="text-muted-foreground hover:text-foreground">
...
export function Sidebar() {
  return <aside className="hidden lg:flex w-64 shrink-0 border-r border-sidebar-border shadow-card"><SidebarContent /></aside>;
}
  ...
  return (
    <header className="lg:hidden sticky top-0 z-40 flex items-center justify-between h-14 px-4 bg-gradient-primary border-b border-sidebar-border shadow-card">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <h1 className="text-sm font-bold text-primary-foreground tracking-tight truncate">GM - GESTÃO GERAL</h1>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-primary-foreground/10" aria-label="Abrir menu">
